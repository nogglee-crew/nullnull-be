...
- Open decisions:
    ...
    - 시간 후보 생성과 장소 후보 생성을 `RoomService` 안에서 직접 orchestration할지, 별도 후보 생성 service/provider로 분리할지 확인 필요
    - 추천 장소 후보 생성에 사용할 외부 장소 검색 API를 확정할 필요가 있음
...
3. 후보 생성 입력 모델을 확정한다.
    ...
    - origin 기반 중간 지점을 계산한 뒤, 그 좌표를 기준으로 외부 장소 검색 API를 호출해 추천 장소 후보를 만든다.
...
5. 장소 후보 생성 유스케이스를 구현한다.
    ...
    - 중간 지점 계산 후 외부 장소 검색 API로 장소 후보 TOP N을 조회하고 저장한다.
...
- 장소 후보 생성은 외부 API 의존이므로 네트워크 실패 시 room 상태를 `READY`로 바꾸지 않고 전체를 실패 처리할지 먼저 정해야 한다.
- `SUBMITTED` participant가 적더라도 `blocked_slots`가 비어 있을 수 있어, 후보 생성 최소 조건을 별도로 정리해야 할 수 있다.
...
- 2026-05-08: 장소 후보는 origin 기반 중간 지점을 계산한 뒤 외부 장소 검색 API로 조회한다.
