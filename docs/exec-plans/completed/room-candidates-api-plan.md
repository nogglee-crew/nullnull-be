...
3. 응답 기준을 확정한다.
    ...
    - 전원 가능이 아닌 후보 카드의 `불가 n명` 및 닉네임 chip을 렌더링할 수 있도록 각 시간 후보에 `unavailableParticipants`를 포함한다.
...
4. 시간 후보 응답 shape를 구현한다.
    ...
    - 응답은 `id`, `date`, `startAt`, `endAt`, `availableCount`, `durationMinutes`, `rank`, `unavailableParticipants`를 반환한다.
    - 프론트 렌더링 혼동을 줄이기 위해 `date`, `startAt`, `endAt`는 KST 기준으로 그대로 해석 가능한 값으로 내려준다.
    - verify: 현재 `time_options` 저장 구조와 응답 구조가 직접 대응됨
    - verify: `availableCount === submittedParticipantCount`이면 전원 가능, 아니면 `unavailableParticipants`로 불가자 목록을 바로 렌더링할 수 있음
...
6. 조회 쿼리와 정렬을 구현한다.
    ...
    - 시간 후보별 불가자 목록을 계산할 수 있도록 `SUBMITTED` participant의 `nickname`, `blocked_slots`도 함께 조회한다.
...
- `submittedParticipantCount`를 조회 시점에 다시 계산할지, room ready 시점 기준 스냅샷처럼 다룰지 결정하지 않으면 `전원 가능 / n명 가능` 판단 기준이 흔들릴 수 있다.
...
- 후보 생성은 room ready 시점에 끝나 있지만, 후보 카드의 불가자 목록은 조회 시점에 `blocked_slots`를 다시 조합해 계산하므로 room ready와 후보 조회의 기준이 어긋나지 않는지 확인해야 한다.
...
- Tests:
    ...
    - 전원 가능이 아닌 시간 후보는 `unavailableParticipants`가 함께 반환됨
- Manual checks:
...
- 2026-05-09: 후보 카드에서 불가자 닉네임을 표시해야 하므로, 후보 조회 API는 각 시간 후보에 `unavailableParticipants`를 계산해 포함한다.
